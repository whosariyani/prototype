...
$employee_type = isset($_POST["employeeType"]) ? trim(htmlspecialchars($_POST["employeeType"] ?? '')) : '';
...
<body>
    ...
    if (isset($_POST['submit'])) {
        try {
            ...
            $employee_type = trim(htmlspecialchars($_POST["employeeType"] ?? ''));
            $employee_number = trim(htmlspecialchars($_POST["employeeNumber"] ?? ''));
            $bday = trim(htmlspecialchars($_POST["birthdate"] ?? ''));
            $age = isset($_POST["age"]) ? (int)$_POST["age"] : 0;
            $gender = trim(htmlspecialchars($_POST["gender"] ?? ''));
            $address = trim(htmlspecialchars($_POST["address"] ?? ''));
            $contact_number = trim(htmlspecialchars($_POST["contactNumber"] ?? ''));
            ...
            elseif (!preg_match("/^\d{10,12}$/", preg_replace("/\D/", "", $contact_number))) {
                ...
                $check_sql = "SELECT * FROM personnel_info WHERE employee_number = ?";
        ...
        <form id="registration-form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
            <div class="form-container">
                ...
                <div class="form-section" id="qr-section">
                    <div class="section-header">
                        <h3><i class="fas fa-qrcode"></i> Personnel QR Code</h3>
                    </div>
                    <div class="section-content">
                        <div class="qr-container">
                            ...
                            <p class="qr-info">This QR code contains the personnel's identification information for easy scanning.</p>
                        </div>
                    </div>
                </div>
    ...
    <div id="success-modal" class="modal">
        <div class="modal-content">
            ...
            <p>The personnel has been successfully registered in the system.</p>
            <div class="qr-download">
                <img id="modal-qr-image" src="/placeholder.svg" alt="Personnel QR Code">
    ...
    <script>
        ...
        // Generate QR Code
        function generateQR() {
            // Get personnel info for QR code
            const employeeNumber = employeeNumberInput.value || '<?php echo isset($_SESSION["employee_number"]) ? $_SESSION["employee_number"] : ""; ?>';
            const personnelName = '<?php echo isset($_SESSION["personnel_name"]) ? $_SESSION["personnel_name"] : ""; ?>';
